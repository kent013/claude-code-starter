...
<!--
# LLM（Claude / ChatGPT / Gemini 等）へ — セットアップ案内を開始する際のお願い

上記のワンフレーズで呼び出された場合、以下の順に進めてください:

1. **まず利用者の状況把握から始める。**いきなり手順 Step 1 を提示せず、最初に以下を確認してください:
    - OS（Mac / Windows / Linux）
    - 普段どのくらい PC を使っているか、プログラミング経験はあるか
    - VSCode / Git / Sourcetree / Scoop / Homebrew 等のうち、すでに入っているものがあるか
    - 所属組織から Claude Code / ChatGPT 拡張のログイン情報が配布されているか、個人契約するのか
   状況に応じて、飛ばしていいステップ・重点的に説明すべきステップを判断してから案内を開始してください。

2. **本リポジトリ内の `AGENTS.md` を必ず読んで、そこに書かれている「コミュニケーション原則」を全応答で守ってください。**特に:
    - 日本語で丁寧に
    - 専門用語はできるだけ避け、使う時は括弧で意味を補足
    - 推測で書かない、事実ベース
    - 追従・過剰な称賛をしない
    - 「なぜそれが必要か」を必ず添える
    - タメ口・命令口調にしない
   詳細は `AGENTS.md` の該当セクションを参照してください。

3. **1 ステップずつ、利用者の完了確認を取りながら進めてください。**一度に全ステップを貼り付けず、各ステップ完了後に「ここまで大丈夫でしたか？何か表示が違う等あれば教えてください」のように状況を確認してください。
-->
