...
- [想定利用者](#想定利用者)
- [最初にやること（セットアップ）](#最初にやることセットアップ)
  - [Step 1: VSCode のインストール](#step-1-vscodeエディタのインストール)
  - [Step 2: Sourcetree のインストール](#step-2-sourcetreegit-の画面操作ツールのインストール)
  - [Step 3: Windows の利用者のみ — Scoop](#step-3-windows-の利用者のみ--scoopソフト管理ツールを入れておく)
  - [Step 4: クローン & VSCode で開く](#step-4-本ボイラープレートを-sourcetree-で取得して-vscode-で開く)
  - [Step 5: 環境構築スキルを実行](#step-5-環境構築スキルを実行)
  - [Step 6: 状態確認](#step-6-状態確認)
- [ディレクトリの使い方](#ディレクトリの使い方)
- [同梱スキル一覧](#同梱スキル一覧)
  - [スキルの呼び出し方](#スキルの呼び出し方重要)
  - [セットアップ・診断](#セットアップ診断)
  - [Codex との合議](#codex-との合議)
  - [Office ファイルの生成・編集](#office-ファイルの生成編集)
  - [ファイル読込（MarkItDown）](#ファイル読込markitdown)
  - [タスク管理](#タスク管理)
  - [MCP（外部サービス連携）のセットアップ](#mcp外部サービス連携のセットアップ)
  - [カスタマイズ](#カスタマイズ)
- [連携できる外部サービス（MCP）](#連携できる外部サービスmcp)
- [困ったとき](#困ったとき)
- [内部仕様・カスタマイズ](#内部仕様カスタマイズ)
...
#### VSCode の日本語化（初めて使う方向け）

VSCode は初期状態だと英語表記です。日本語メニューにしたい場合は以下の手順:

1. VSCode を起動
2. 左サイドバーの一番下にある「拡張機能」アイコン（四角いパズルのピースのマーク）をクリック
3. 検索欄に `Japanese Language Pack` と入力
4. 検索結果の **`Japanese Language Pack for Visual Studio Code`（Microsoft 製、公式）** の右にある青い「Install」ボタンをクリック
5. 右下に「言語を切り替えて再起動する」のような通知が表示されるので、「Change Language and Restart」ボタンをクリック
6. VSCode が再起動し、メニューが日本語になります

英語のままでも本ボイラープレートは使えますが、初めて使う方は日本語化したほうが操作に迷いにくいです。

### Step 2: Git と Sourcetree（Git の画面操作ツール）のインストール
...
Windows では **先に「Git for Windows」をインストール**してください。Claude Code 拡張（VSCode 内の拡張機能）は内部で `git-bash`（Git for Windows に付属する Bash シェル）を必要とするため、これが入っていないと後の Step 4 で Claude Code を起動しようとした時に次のようなエラーが出ます:
...
1. **Git for Windows をダウンロード**: <https://git-scm.com/downloads/win> から 64-bit 版をダウンロードして実行
2. インストール途中の選択肢はすべて初期値のままで OK（Enter 連打で進めて問題ありません）
3. インストール完了後、**VSCode を一度完全に終了して再起動**してください（PATH 変更を反映するため）
4. それでも同じエラーが出る場合は、環境変数 `CLAUDE_CODE_GIT_BASH_PATH` に `C:\Program Files\Git\bin\bash.exe` を設定してください（VSCode を再起動）

この手順が終わると Claude Code 拡張が起動できるようになります。

続けて **Sourcetree** をインストール:

- ダウンロード: <https://www.sourcetreeapp.com/>
- Atlassian アカウント（無料）でのサインインを求められたら、画面の指示に従ってください（Google アカウント等でもサインインできます）

#### Mac の方

- Sourcetree をダウンロード・インストール: <https://www.sourcetreeapp.com/>
- Atlassian アカウント（無料）でのサインインを求められたら、画面の指示に従ってください
- Mac は標準で Git 本体がコマンドラインに入っているので、別途インストール不要です

---

※ コマンド操作に慣れている方は Sourcetree を使わず、`git` コマンド直接でも構いません。
...
インストールが終わったら、PowerShell を **一度閉じて開き直して** ください（`scoop` コマンドが認識されるようになります）。この Step 3 が完了したら **PowerShell は閉じてしまって構いません**（以降の操作は VSCode の中で完結します）。
...
1. **Sourcetree でクローン**:
   ...
   - ダウンロードが終わったら **Sourcetree は閉じて構いません**（以降の操作は VSCode の中で完結します。将来的にブランチ操作などをしたい時にまた開けば OK）
...
スキルは **日本語での自然なお願い** で呼び出せます。スラッシュコマンドを覚える必要はありません。

Claude Code は会話開始時に「このプロジェクトにどんなスキルがあるか」を自動で読み込むので、利用者が自然に話しかけると、**その内容に合ったスキルを自動で起動** します。

**全スキルの発言例:**

| 利用者の発言例 | 起動されるスキル |
|---|---|
| 「初期セットアップして」「環境構築をお願いします」「最初の設定をして」 | `claude-code-starter-setup` |
| 「環境を診断して」「動作確認して」「doctor して」「セットアップできてるか見て」 | `claude-code-starter-doctor` |
| 「この件について Codex にも意見を聞いてみて」「Codex と議論して」「別の視点が欲しい」 | `claude-code-starter-codex-review` |
| 「GitHub 連携を設定したい」「Backlog のトークンを設定して」「Google Drive と繋ぎたい」 | `claude-code-starter-mcp-setup` |
| 「これを TODO に追加して」「やることリストに入れておいて」 | `claude-code-starter-todo-add` |
| 「この TODO を完了にして」「あのタスクは不要になったので消して」 | `claude-code-starter-todo-close` |
| 「自分用のスキルを作りたい」「{タスク}を毎回お願いするの面倒なのでテンプレ化したい」 | `claude-code-starter-skill-add` |
| 「このセッションのログを報告用に出力して」「○○について話してたセッションを送れるようにして」 | `claude-code-starter-session-export` |
| 「企画書を PowerPoint で作って」「このスライドに表を追加して」「pptx の 3 枚目を差し替えて」 | `pptx`（公式 Office スキル） |
| 「売上データの Excel を作って」「この xlsx にピボットテーブルを追加して」「CSV を整形して xlsx にして」 | `xlsx`（公式 Office スキル） |
| 「議事録を Word 文書にして」「この docx に目次を入れて」「レターヘッド付きの Word を作って」 | `docx`（公式 Office スキル） |
| 「この報告を PDF にまとめて」「PDF を 2 つ結合して」「PDF の 3〜5 ページを抜き出して」 | `pdf`（公式 Office スキル） |
| 「このファイルの中身を要約して」「pptx から本文を抜き出して」「zip の中の議事録を一覧化して」「この PDF を Markdown 化して」 | MarkItDown（`uv run --with markitdown[all] markitdown`） |

**スラッシュコマンドも使えます**（明示的にスキルを指定したい時のショートカット）:

```
/claude-code-starter-doctor
```

どちらの方法でも同じスキルが動きます。普段は自然な日本語で話しかけて OK です。

---

以下、このボイラープレートに同梱されているスキルの一覧です。
...
### MCP（外部サービス連携）のセットアップ

| スキル | 用途 |
|--------|------|
| `claude-code-starter-mcp-setup` | 個別の MCP サービス（GitHub / Backlog / Google Drive / DocBase / Context7）のセットアップを対話的にサポート |
